...
class LachesisSchedulerTwo(
  ...
  val schedulingState = LachesisSchedulerTwoState()(lbConfig)

  /**
   * Monitors invoker supervision and the cluster to update the state sequentially
   *
   * All state updates should go through this actor to guarantee that
   * [[LachesisSchedulerTwoState.updateInvokers]] and [[LachesisSchedulerTwoState.updateCluster]]
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    val hash = LachesisSchedulerTwo.generateHash(msg.user.namespace.name, action.fullyQualifiedName(false))
    ...
    val (chosen, finalFqn: FullyQualifiedEntityName, memSlots: Int, cpuCores: Int) = if (invokersToUse.nonEmpty) {
      val (invoker: Option[(InvokerInstanceId, Boolean)], function: FullyQualifiedEntityName, slots: Int, cores: Int) = LachesisSchedulerTwo.schedule(
        action.limits.concurrency.maxConcurrent,
        action.fullyQualifiedName(true),
        invokersToUse,
        schedulingState.invokerCoreLoads,
    ...
    chosen
      .map { invoker =>
        ...
        // Update invoker load state
        schedulingState.increaseInvokerCoreLoad(invoker.toInt, action.limits.cpu.cores)
  ...
  override protected def releaseInvoker(invoker: InvokerInstanceId, entry: ActivationEntry) = {
    ...
    schedulingState.decreaseInvokerCoreLoad(invoker.toInt, entry.originalcpuLimit.toInt)
  }
}

object LachesisSchedulerTwo extends LoadBalancerProvider {
  ...
  def schedule(
    ...
    invokerCoreLoads: IndexedSeq[Int],
    ...
    chosenIndexLoad: Int = -1,
    leastLoadedIndex: Int = -1,
    finalRule: Int = -1)(implicit logging: Logging, transId: TransactionId): (Option[(InvokerInstanceId, Boolean)], FullyQualifiedEntityName, Int, Int) = {
        ...
        if (numInvokers > 0) {
            if (stepsDone == numInvokers) {
                ...
                // Case 8: None of the invokers have any space choosen least loaded
                } else {
                    logging.warn(this, s"Lachesis launching: activation_id ${activationId} in rule 8 on least loaded index ${leastLoadedIndex}")
                    ...
                    // }
                    (Some(invokers(leastLoadedIndex).id, true), fqn, slots, cores)
                }
            } else {
                // Current state
                ...
                var bestIndexLoad = chosenIndexLoad
                var currentRule = finalRule

                // Get the invoker and invoker map and invoker core load for current recursive call we are in
                val invoker = invokers(index)
                val invokerMap = containerMap(index)
                val invokerCoreLoad = invokerCoreLoads(index)
                ...
                if (!invokerMap.isEmpty) {
                    ...
                    for ((mapFqn, count) <- invokerMap) {
                        ...
                        if (mapFqnName == currFqnName) {
                            ...
                            // Found warm container that is perfect size on this invoker
                            if (diffCpu == 0 && diffMem == 0 && count > 0) {

                                // Found a previous warm container of perfect size
                                if (bestDiffCpu == 0 && bestDiffMem == 0) {
                                    // Check if the load of the invoker previous perfect warm container is on is less than the current invoker loads
                                    if (bestIndexLoad < invokerCoreLoad) {
                                        // Check if we can reserve resrouces on this invoker for the current container we are considering
                                        if (getResources(invoker, dispatchedMem, dispatchedCpu, mapFqn, maxConcurrent, slots, cores)) {
                                            // Release resources of the previous perfectly warm container size we found
                                            ...
                                            bestDiffMem = 0
                                            bestFqn = mapFqn
                                            bestIndexLoad = invokerCoreLoad
                                            currentRule = 1 // Case 1: Found perfect warm container on an invoker that is more densely packed
                                        }
                                    }
                                }
                                // Check if we can reserve resources on this invoker for the current container we are considering
                                else if (getResources(invoker, dispatchedMem, dispatchedCpu, mapFqn, maxConcurrent, slots, cores)) {
                                    ...
                                    bestFqn = mapFqn
                                    bestIndexLoad = invokerCoreLoad
                                    currentRule = 2 // Case 2: Found perfect warm container for the first time, don't care about densely packing
                                }
                            }

                            // Found a warm container that is closest in memory and has enough cores compared to the requested size
                                ...
                                if (getResources(invoker, dispatchedMem, dispatchedCpu, mapFqn, maxConcurrent, mapFqnMem, cores)) {
                                    ...
                                    bestFqn = mapFqn
                                    bestIndexLoad = invokerCoreLoad
                                    currentRule = 3 // Case 3: Found a larger container best in memory and satisfactory in CPU, don't care about packing
                                }
                            }
                            
                            // Found a container equal in memory to the closest we've seen, so see if it's closer in cores
                                ...
                                if (getResources(invoker, dispatchedMem, dispatchedCpu, mapFqn, maxConcurrent, mapFqnMem, cores)) {
                                    ...
                                    bestFqn = mapFqn
                                    bestIndexLoad = invokerCoreLoad
                                    currentRule =  4 // Case 4: Found a larger container that is best in memory and better in CPU, don't care about packing
                                }
                            }

                            // Found a container equal in memory and CPU to the closest we've seen, so now decide which to use based on packing
                            else if (diffMem >= 0 && diffMem == bestDiffMem && diffCpu >= 0 && diffCpu == bestDiffCpu && count > 0 && bestIndexLoad < invokerCoreLoad) {
                                // Check if we can reserve resources on this invoker for the current container we are considering
                                if (getResources(invoker, dispatchedMem, dispatchedCpu, mapFqn, maxConcurrent, mapFqnMem, cores)) {
                                    // Check if we reserved resurces on this invoker or a previous invoker, if so release those resoruces
                                    if (bestIndex != -1) {
                                        dispatchedMem(invokers(bestIndex).id.toInt).releaseConcurrent(bestFqn, maxConcurrent, bestFqn.name.toString.split("_")(2).toInt)
                                        dispatchedCpu(invokers(bestIndex).id.toInt).releaseConcurrent(bestFqn, maxConcurrent, cores)
                                    }
                                    ...
                                    bestFqn = mapFqn
                                    bestIndexLoad = invokerCoreLoad
                                    currentRule =  5 // Case 5: Found a larger container of same size from the previous we found, decide which to use based on packing
                                }
                            }
                        }
                    }

                    // Haven't found an invoker with perfect or larger warm container (i.e., current invocation would experience a cold start)
                    // so check if current invoker load is greater than previous, if so, pack the cold start on this invoker node
                    if ((currentRule == -1 || currentRule > 5) && bestIndexLoad < invokerCoreLoad) {
                        if (getResources(invoker, dispatchedMem, dispatchedCpu, fqn, maxConcurrent, slots, cores)) {
                            bestIndex = index
                            bestIndexLoad = invokerCoreLoad
                            currentRule = 6 // Case 6: Haven't found a suitable warm container, so schedule cold start on current invoker because better packing
                        }
                    }

                } else {
                    // Haven't found an invoker with a perfect or larger warm container, so check if current invoker has space for requested allocation
                    if ((currentRule == -1 || currentRule > 5) && bestIndexLoad < invokerCoreLoad) {
                        if (getResources(invoker, dispatchedMem, dispatchedCpu, fqn, maxConcurrent, slots, cores)) {
                            bestIndex = index
                            bestIndexLoad = invokerCoreLoad
                            currentRule = 7 // Case 7: Haven't found a suitable warm container, so schedule cold start on current invoker because better packing
                        }
                    }
                }

                // Track least loaded index in case all the invokers don't have space
                var bestLeastLoadedIndex = leastLoadedIndex
                if (bestLeastLoadedIndex == -1) {
                    bestLeastLoadedIndex = index
                } else if (invokerCoreLoad < invokerCoreLoads(bestLeastLoadedIndex)) {
                    bestLeastLoadedIndex = index
                }

                
                val newIndex = (index + step) % numInvokers
                schedule(maxConcurrent, fqn, invokers, invokerCoreLoads, containerMap, dispatchedMem, dispatchedCpu, slots, cores, newIndex, step, bestFqn, activationId, stepsDone + 1, bestIndex, bestDiffCpu, bestDiffMem, bestIndexLoad, bestLeastLoadedIndex, currentRule)
            }  
        } else {
            (None, fqn, slots, cores)
        }
 ...
 */
case class LachesisSchedulerTwoState(
  ...
  private var _managedStepSizes: Seq[Int] = LachesisSchedulerTwo.pairwiseCoprimeNumbersUntil(0),
  private var _blackboxStepSizes: Seq[Int] = LachesisSchedulerTwo.pairwiseCoprimeNumbersUntil(0),
  private var _invokerCoreLoads: IndexedSeq[Int] = IndexedSeq.empty[Int],
  ...
  def invokerCoreLoads: IndexedSeq[Int] = _invokerCoreLoads
  ...
  def increaseInvokerCoreLoad(invokerId: Int, increaseCoreLoadValue: Int) {
    this.synchronized {
        _invokerCoreLoads = _invokerCoreLoads.updated(invokerId, _invokerCoreLoads(invokerId) + increaseCoreLoadValue)
    }
  }

  def decreaseInvokerCoreLoad(invokerId: Int, decreaseCoreLoadValue: Int) {
    this.synchronized {
        _invokerCoreLoads = _invokerCoreLoads.updated(invokerId, _invokerCoreLoads(invokerId) - decreaseCoreLoadValue)
    }
  }
  ...
  def updateInvokers(newInvokers: IndexedSeq[InvokerHealth]): Unit = {
    ...
    if (oldSize != newSize) {
        _invokerCoreLoads = IndexedSeq.fill(newSize)(0)
    }
    ...
    val logDetail = if (oldSize != newSize) {
      _managedStepSizes = LachesisSchedulerTwo.pairwiseCoprimeNumbersUntil(managed)
      _blackboxStepSizes = LachesisSchedulerTwo.pairwiseCoprimeNumbersUntil(blackboxes)
      ...
      if (oldSize < newSize) {
        ...
        _invokerCores = _invokerCores ++ onlyNewInvokers.map { invoker =>
          new NestedSemaphore[FullyQualifiedEntityName](70)
        }
        ...
        val newInvokerDetails = onlyNewInvokers
          .map(i =>
            s"${i.id.toString}: ${i.status}  | ${getInvokerSlot(i.id.userMemory).toMB.MB} of ${i.id.userMemory.toMB.MB} | 70 cores available for Invoker")
          .mkString(", ")
  ...
  def updateCluster(newSize: Int): Unit = {
    ...
    if (_clusterSize != actualSize) {
      ...
      val totalInvokerCore = 
        _invokers.foldLeft(0L)((total, invoker) => total + 70)
